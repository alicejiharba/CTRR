...
// PathNode* findShortestPathMatrix(double adjMatrix[100][100], int start, int goal) {
//     // Viết code thuật toán A* cho ma trận kề ở đây
//     // cout <<"Running A* on adjacency matrix from " << start << " to " << goal << endl;
//     int nVertices = 100;
//     for (int i = 99; i >= 0; i--) {
//         bool nonZero = false;
//         for (int j = 0; j < 100; j++) {
...
//     // 2. BFS heuristic: h(n) = số bước ít nhất để tới goal
//     auto calcHeuristic = [&](int src) {
//         if (src == goal) return 0;
...
//             }
//         }
//         return 1000000; // không tới được → heuristic = "rất lớn"
//     };

//     // Precompute heuristic cho tất cả node
//     double h[100];
//     for (int i = 0; i < nVertices; i++)
//         h[i] = calcHeuristic(i);
...
//     while (true) {
//         // Tìm node có f nhỏ nhất trong open set
//         int current = -1;
//         double bestF = 1e18;
...
//             while (v != -1) {
//                 PathNode* newNode = createPathNode(
//                     to_string(v),
//                     f[v],
//                     g[v],
//                     h[v]
...
//             double tentative = g[current] + 1; // cost = 1

//             if (!inOpen[nb] || tentative < g[nb]) {
//                 parent[nb] = current;
//                 g[nb] = tentative;
//                 f[nb] = g[nb] + h[nb];
//                 inOpen[nb] = true;
...
// int BFS(int maze[100][100], int m, int n, int startX, int startY, int goalX, int goalY, vector<pair<int, int>>& path) {
//     // Viết code BFS để tìm đường đi trong mê cung
//     int dx[4] = {-1, 1, 0, 0};
//     int dy[4] = {0, 0, -1, 1};
...
//     // ---- HÀNG ĐỢI TỰ TẠO ----
//     pair<int,int> q[10000]; // đủ lớn cho BFS trong 100x100
//     int front = 0, back = 0;

//     // Đưa điểm bắt đầu vào queue
//     q[back++] = {startX, startY};
...
//     // Convert (x, y) → index và ngược lại
...
//         // Lấy node có f nhỏ nhất trong open set
//         for (int i = 0; i < totalNodes; i++) {
//             if (inOpen[i] && !closed[i] && f[i] < bestF) {
//                 bestF = f[i];
//                 current = i;
...
//     // Chuyển mê cung sang đồ thị và tìm đường
//     auto inBounds = [&](int x, int y) {
//         return x >= 0 && x < m && y >= 0 && y < n && maze[x][y] == 0;
//     };
...
//     // ================================
//     // Heuristic Manhattan * minWeight
//     // ================================
//     double minW = 1e9;
//     for (int i = 0; i < m; i++)
//         for (int j = 0; j < n; j++)
//             if (maze[i][j] == 0)
//                 minW = min(minW, weightMatrix[i][j]);

//     auto heuristic = [&](int id) {
//         int x = cx(id), y = cy(id);
//         double manh = abs(x - goalX) + abs(y - goalY);
//         return manh * minW;
//     };
...
//     bool inOpen[10000] = {false};
//     bool closed[10000] = {false};
//     int parent[10000], moveDir[10000];
...
//             while (v != -1) {
//                 string name;
//                 if (v == start) name = "Start";
//                 else name = dirName[moveDir[v]];
...
//             double costToEnter = baseCost[d] * weightMatrix[nx][ny];

//             double tentative_g = g[current] + costToEnter;
